...
WRAPPER="$PROJECT_ROOT/ui.sh"
TEST_ROOT="$(mktemp -d "${TMPDIR:-/tmp}/md-autotax-wrapper-test.XXXXXX")"
...
make_project() {
    ...
    chmod +x "$project_dir/venv/bin/streamlit"
}

help_home="$TEST_ROOT/help-home"
HOME="$help_home" "$WRAPPER" --help > "$TEST_ROOT/help.out"
grep -q 'MD_AUTOTAX_PROJECT_DIR' "$TEST_ROOT/help.out" || fail "help omits environment override"
[[ ! -e "$help_home/.md-autotax" ]] || fail "help created runtime state"

colocated="$TEST_ROOT/colocated project"
make_project "$colocated"
cp "$WRAPPER" "$colocated/ui.sh"
"$colocated/ui.sh" --check > "$TEST_ROOT/colocated.out"
...
standalone="$TEST_ROOT/Library/Scripts/md-autotax-ui"
configured_project="$TEST_ROOT/configured autotax project"
...
MD_AUTOTAX_HOME="$runtime_home" "$standalone" --check > "$TEST_ROOT/standalone.out"
...
env -u HOME MD_AUTOTAX_PROJECT_DIR="$configured_project" \
...
MD_AUTOTAX_HOME="$runtime_home" "$standalone" > "$TEST_ROOT/launch.out"
grep -Fq "cwd=$configured_canonical" "$TEST_ROOT/launch.out" || fail "launcher did not use project working directory"
grep -Fq "args=run $configured_canonical/app.py --server.address 127.0.0.1" \
    "$TEST_ROOT/launch.out" || fail "launcher invoked Streamlit incorrectly"

unsafe_home="$TEST_ROOT/unsafe-home"
marker="$TEST_ROOT/SHOULD_NOT_EXIST"
mkdir -p "$unsafe_home"
printf '%s\n%s\n' "$configured_project" "\$(touch $marker)" > "$unsafe_home/project-dir"
if MD_AUTOTAX_HOME="$unsafe_home" "$standalone" --check > /dev/null 2> "$TEST_ROOT/unsafe.err"; then
...
[[ -f "$first_home/.md-autotax/project-dir" ]] || fail "first run did not create empty config"
[[ ! -s "$first_home/.md-autotax/project-dir" ]] || fail "first-run config was not empty"
grep -q 'created empty local config' "$TEST_ROOT/first.err" || fail "first-run error was not actionable"

printf 'md-autotax wrapper tests: PASS\n'
