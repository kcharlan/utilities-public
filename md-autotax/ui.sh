...
TOOL_NAME="md-autotax"
PROJECT_DIR_ENV_NAME="MD_AUTOTAX_PROJECT_DIR"
RUNTIME_HOME_ENV_NAME="MD_AUTOTAX_HOME"
EXPECTED_APP="app.py"
...
Launch the MD AutoTax Streamlit UI from its project environment.

Project directory resolution, in priority order:
  1. MD_AUTOTAX_PROJECT_DIR
  2. The directory containing this wrapper, when it contains the project
  3. First line of ${MD_AUTOTAX_HOME:-~/.md-autotax}/project-dir

Options:
  --check   Validate configuration and required files without starting Streamlit.
  -h, --help
...
read_project_dir_file() {
    ...
    [[ $line_count -eq 1 && -n "$candidate" ]] || fail \
        "$config_file is empty; write the absolute md-autotax project path on its only line."
    printf '%s\n' "$candidate"
}
...
resolve_project_dir() {
    ...
    if [[ -n "${MD_AUTOTAX_PROJECT_DIR:-}" ]]; then
        candidate="$MD_AUTOTAX_PROJECT_DIR"
    elif [[ -f "$wrapper_dir/$EXPECTED_APP" ]]; then
    ...
    [[ -f "$candidate/$EXPECTED_APP" ]] || fail \
        "configured project directory is missing $EXPECTED_APP: $candidate"
    [[ -x "$candidate/venv/bin/streamlit" ]] || fail \
        "project environment is missing executable venv/bin/streamlit: $candidate"
    printf '%s\n' "$candidate"
}
...
cd "$PROJECT_DIR"
exec "$PROJECT_DIR/venv/bin/streamlit" run "$PROJECT_DIR/$EXPECTED_APP" \
