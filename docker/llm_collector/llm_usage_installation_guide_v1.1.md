...
- **Node.js 18+** (for extension validation run by `migrate.sh`)
...
- Send a prompt
- Click the extension icon → the collector indicator should turn green and counts should appear
...
| Red collector indicator | Read the popup’s specific error, then confirm Docker is running and the generated extension API key is current |
| Server not reachable | Confirm Docker container is running: `docker ps`, then use **Reload** in the extension popup |
