...
> **Usage:** Start a new session and say:
>
> ```
> Read task_orch/planning/INTAKE_PROMPT.md and convert the following list of items into work intake documents:
>
> - <item 1 description>
> - <item 2 description>
> - ...
...
intake documents for an agent pipeline. Each intake document will be picked up
by a planning agent (cc-opus), which will read it and produce a detailed
implementation plan. The planner is smart but has no context beyond what you
write — so each document must be self-contained with enough context for the
planner to find the right code and make good decisions.

## Before writing anything

1. **Read these files** (in this order):
   - `CLAUDE.md` (repo root) — project conventions, test commands, branch workflow
   - `task_orch/SYSTEM.md` — pipeline rules, directory structure, file lifecycle
   - `task_orch/USER_PROCESS.md` — the "Intake item format" section specifically
   - `task_orch/planning/PLANNER.md` — understand what the planner expects to receive
...
Each intake document is a markdown file placed in `task_orch/planning/intake/`.

**Filename:** `<NNN>_<short_snake_case_slug>.md`
- NNN = zero-padded sequence number (001, 002, ...)
- **Get the next number from `task_orch/planning/NEXT_SEQUENCE`.** This file contains
  a single integer — the next available sequence number. Read it, use that number,
  then write the incremented value back. This is the only reliable way to get the
  next number — do NOT search directories or guess.
- As a defensive check, verify no file with that number already exists anywhere
  under `task_orch/`. If a collision is found, increment until clear and update
  `NEXT_SEQUENCE` accordingly.
...
## Quality standards for intake documents
...
- Do not put files anywhere other than `task_orch/planning/intake/`
