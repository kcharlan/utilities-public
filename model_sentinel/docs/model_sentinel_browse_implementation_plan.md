...
- `run_browse_command` validates `--provider` with the existing `validate_selected_providers`, calls `open_readonly`, maps `MissingDatabaseError` → `SystemExit(2)` with `Model Sentinel database not found at <path>. Run 'model-sentinel scan --save' first.` and `SchemaError` → `SystemExit(2)` naming the missing object, then delegates to `browse.server.run_browse(...)`. For this task `run_browse` is a stub returning 0.
